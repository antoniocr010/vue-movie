  ...
  <div class="movie-D">
    <v-container>
      <v-row>
        ...
        <v-col cols="12" sm="8">
          <h1 class="grey-darken-3 mt-2">{{ this.movie.title }}</h1>
          <v-row>
            <v-col cols="12" sm="2">
              <v-rating
          ...
          <v-btn tile color="green accent-3" class="ml-2" @click="addToMyList">
            <v-icon left>mdi-format-list-bulleted </v-icon>AddList
          </v-btn>
        </v-col>
      </v-row>
      <v-divider class="mt-2"></v-divider>
      <CastVue :casts="movie.credits.cast" />
    </v-container>
  </div>
</template>
<script>
import CastVue from '../components/CastVue.vue';
export default {
  components: {
    CastVue,
  },
  ...
  methods: {
    async fetchMovie(movieId) {
      ...
      const resp = await this.$http.get(
        '/movie/' + movieId + key + '&append_to_response=credits,videos,images'
      );
    ...
    showPad() {
      window.Toast.fire('Success', 'Successfully added!', 'success');
    },
  },
};
</script>
